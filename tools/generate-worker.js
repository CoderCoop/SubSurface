...
function search(n, seed, tries, digR, keep) {
  ...
  for (let i = 0; i < tries && accepted < enough; i++) {
    ...
    /*
     * Judge the spec EXACTLY as it will be banked. The bank rounds every
     * dial to five decimals on save, so an unrounded sample is a slightly
     * different level from the one the file will rebuild — usually
     * immaterially, but the criterion lives at knife edges: level 29 was
     * accepted with its best naive drop under 85% and verified back at
     * 89.6% after the round trip. Rounding before judging closes the gap
     * for good instead of hoping the edge is never this close again.
     */
    for (const k of Object.keys(spec))
      if (typeof spec[k] === 'number' && !Number.isInteger(spec[k]))
        spec[k] = Number(spec[k].toFixed(5));
    looked++;
    ...
    const near = (v) =>
      ...
      ((v.ace &&
        ...
        /*
         * The other kind of close: no ace, but the best plan lands within a
         * few points of the top tier. On the deep levels this is the COMMON
         * near-miss — the lane threads everything and arrives at 94–96 —
         * and the old predicate never climbed from it, because it required
         * the ace the sample was one nudge short of. A basin or slope a
         * shade friendlier is exactly what perturbation finds. Measured at
         * level 25: three 40-try seeds without this found nothing; the
         * first 21-try seed with it banked the level.
         */
        (!v.ace && (v.best || 0) >= 94));
